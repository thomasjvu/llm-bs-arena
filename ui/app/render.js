...
import { EXPERIMENTS, getQuotedExperimentSummary } from './experiments.js';

const RESEARCH_DASHBOARD_URL = '/research/';
  ...
  container.innerHTML = experiment
    ? `
      <article class="experiment-faq-item experiment-faq-item--single is-selected">
        ...
        <div class="experiment-faq-line">${getQuotedExperimentSummary(experiment.id)}</div>
...
function renderLauncher(dom, app) {
  ...
  const nimOption = dom.providerSelect.querySelector('option[value="nim"]');
  if (nimOption) {
    nimOption.disabled = app.liveProviderAvailable === false;
  }
  ...
  dom.apiKeyInput.placeholder = maskedServerKey ? '********' : 'session only';
  ...
  if (interactive) {
    setText(
      dom.launcherNote,
      app.provider === 'nim'
        ? (app.serverApiKeyAvailable ? 'Server key loaded.' : 'Session API key; not stored.')
        : (app.liveProviderAvailable === false ? 'Public demo with mock opponents.' : 'Mock table for practicing manual turns.')
    );
  } else {
    setText(
      dom.launcherNote,
...
export function renderApp(dom, app, layout, onToggleCard) {
  ...
  if (dom.researchStatsLink) {
    dom.researchStatsLink.href = RESEARCH_DASHBOARD_URL;
  }
  ...
  setText(dom.soundToggleBtn, 'SOUND');
  dom.soundToggleBtn?.classList.toggle('is-sound-off', !app.soundEnabled);
  dom.soundToggleBtn?.setAttribute('aria-pressed', app.soundEnabled ? 'true' : 'false');
  dom.soundToggleBtn?.setAttribute('aria-label', app.soundEnabled ? 'Sound on' : 'Sound off');
