...
\subsection{Four-Card Zero-Match Bluff Under Asymmetric Fairness}
...
This is the clearest Experiment~2 counterexample because it is a four-card zero-match bluff from the one model that becomes more deceptive under the asymmetric-fairness framing. In the local sequence, Turn~37 is a false challenge against MiniMax's truthful two-Jack play, Turn~38 goes unchallenged, and Turn~40 is a correctly challenged Nemotron four-Ace bluff. That pattern is consistent with uneven enforcement under the softer aggregate inspection regime in Experiment~2. The frozen turn CSV preserves the acting model's reasoning text, but it does not preserve comparable non-challenge reasoning for the players who let Turn~38 pass, so we do not claim direct evidence of their internal rationale.
