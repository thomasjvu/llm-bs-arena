...
The primary statistical unit is one row per player per game rather than one row per turn. Experiment-level narrative claims use mean model-level rates computed from the frozen per-condition summaries. Model-profile sections use the exact per-model condition rates from those same frozen summaries. This keeps cross-condition comparisons from being driven by raw row volume while preserving exact model-level leaderboards. Qualitative reasoning traces and exemplar games are used as supporting evidence rather than as the main statistical unit.
